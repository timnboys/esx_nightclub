...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations2) do
			...
			TaskPlayAnim(npc,"mini@strip_club@idles@stripper","stripper_idle_01",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations3) do
			local npc = CreatePed(1, 0xc79f6928, item.x, item.y, item.z, item.heading, false, true)
			...
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_cheering_female_c",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations4) do
			local npc = CreatePed(1, 0x36df2d5d, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@peds@")
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_partying_female_partying_beer_base",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations5) do
			local npc = CreatePed(1, 0x2f4aec3e, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@peds@")
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","mini_strip_club_private_dance_idle_priv_dance_idle",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations6) do
			local npc = CreatePed(1, 0x303638a7, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@peds@")
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","mini_strip_club_lap_dance_ld_girl_a_song_a_p1",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations7) do
			local npc = CreatePed(1, 0x9d0087a8, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@peds@")
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_partying_male_partying_beer_base",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations8) do
			local npc = CreatePed(1, 0xe6ac74a4, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@peds@")
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_drinking_beer_male_base",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations9) do
			local npc = CreatePed(1, 0xda116e7e, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@peds@")
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_hang_out_street_male_c_base",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations10) do
			local npc = CreatePed(1, 0x780c01bd, item.x, item.y, item.z, item.heading, false, true)
...
Citizen.CreateThread(function()
	...
	if Config.EnableSolomun then
		for _, item in pairs(Config.Locations12) do
			local npc = CreatePed(1, 0x4EF92B55, item.x, item.y, item.z, item.heading, false, true)
			...
			TaskPlayAnim(npc,"anim@amb@nightclub@djs@solomun@","sol_dance_a_sol",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableDixon then
		for _, item in pairs(Config.Locations13) do
			local npc = CreatePed(1, 0xEBE02D05, item.x, item.y, item.z, item.heading, false, true)
			...
			SetPedCanPlayAmbientAnims(npc, true)
			RequestAnimDict("anim@amb@nightclub@djs@solomun@")
			TaskPlayAnim(npc,"anim@amb@nightclub@djs@solomun@","sol_idle_left_e_sol",1.0, 0.0, 500, 1, 10, false, false, false)
		end
	end
end)